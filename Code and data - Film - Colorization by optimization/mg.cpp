...
void MG::add_prolonged_prev_level(int level) {
	...
	restrict(_P[level],tpP);	//Create tpP as half _P[level]
	tpP-=_P[level+1];			//tpP = restrict(_P[level]) - _P[level+1])
	prolong(tpP, pP);		
	_P[level] -= pP ;			//_P[level] = _P[level] - prolong(restrict(_P[level]) - _P[level+1])
}	
...
void MG::prolong(Tensor3d& P, Tensor3d& pP) {
    ...
    int ppx = nx * 2 ;
    int ppy = ny * 2 ;
    int ppz = nz ; // Do not prolong in time!
    ...
    for (int z=0; z<nz; z++){
		...
		// Coordonates in the original image
		int xy = 0 +z*px*py; // The point itself 
		int xpy = 1 +z*px*py; //+1 along x
		int xyp = px +z*px*py; //+1 along y
		int xpyp = 1 + px +z*px*py ;//+1 along x and +1 along y
		int x,y;

		// Heart
		for(y = 1 ; y < py ; y++) {
			for(x = 1 ; x < px ; x++) {
				// Set the 4 new points in the prolonged image as a linear combinaison of the original points
...
void MG::restrict(Tensor3d& P, Tensor3d& rP) {
 // As oppose to prolonge, makes a image of 50% the original size (simple 25% of the 4 previous points)
	int nx = P.len_x ;
	int ny = P.len_y ;
	int nz = P.len_z;
