...
// Tensor2d contains a lx * ly matrix. The data can be reals (we have a function taking the fabs of the datas).
...
// Swap images (possibly of different sizes)
...
//Image becomes equal to other (without copying)
...
// Creates a copy of other as the Image
...
// We assume same len_x (if else ???) and then create a combination of t1 and t2 (t1 above t2)
...
//If not already correctly done, allocate the memory space for the image of size lx,ly - clear any previous data allocation
...
// The same is the data is allready loaded (we assume datap is of size lx * ly
...
// Set all the image with the same "color" val
...
//Makes a 50% * 50% resized image looking at all 8 points around
void Tensor2d::restrict_full(Tensor2d& r) { 
  r.set((len_x - 1) / 2 + 1, (len_y - 1) / 2 + 1) ;
   
  int i, j,x,y;

  // The new 1/2 size image is created out of the original one
...
// Makes a 50% * 50% resized image looking at all 4 points around
...
// Makes a 50% * 50% resized image looking only at 1 point
...
void Tensor2d::prolong_full(Tensor2d& p) {
  ...
  for( y = 1 ; y < len_y ; y++) { // Heart of the image
    for( x = 1 ; x < len_x ; x++) {
...
// Makes a 200% * 200% resized image : each point in the smaller image in copied 4 times in the bigger one
...
//Loads an image de taille lx,ly
...
//Give the maximum of fabs on all the image
...
//Put all the data in image at power c
...
//Multiplication by val of all the data - return the objet pointed by this
...
//Add other to this (if this is smaller, the operation is only performed on a part of this) - return the objet pointed by this
...
//Substract other to this (if this is smaller, the operation is only performed on a part of this) - return the objet pointed by this
Tensor2d& Tensor2d::operator-=(Tensor2d& other) {
...
// Give the mean value over all the points of fabs(data[i])
...
// Give the mean value over all the points of (data[i])**2
...
// Give the minimum value over all the points
real Tensor2d::min() 
{
   double min = 1e50 ;
...
// Give the maximum value over all the points
real Tensor2d::max() 
{
   double max = -1e50 ;
