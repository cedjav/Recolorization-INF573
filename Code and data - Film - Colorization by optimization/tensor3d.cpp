...
Tensor3d& Tensor3d::operator-=(Tensor3d& other) {
	...
	if (len_x < other.len_x) xmin = len_x; else xmin = other.len_x;
	if (len_y < other.len_y) ymin = len_y; else ymin = other.len_y;

	for (int i = 0; i < xmin; i++)
		for (int j = 0; j < ymin; j++) {
			data[i + j * len_x] -= other.data[i + j * other.len_x];
		}

	if (len_x>other.len_x)
		for (int j = 0; j < ymin; j++) 
			data[len_x -1 + j * len_x] = 2*data[len_x - 2 + j * len_x]-data[len_x - 3 + j * len_x];
